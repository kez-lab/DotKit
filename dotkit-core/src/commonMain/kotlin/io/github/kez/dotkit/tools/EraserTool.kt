 ...
 * 지우개 도구
 * 픽셀을 투명하게 만듭니다 (알파 = 0).
 * 드래그하면 연속적으로 픽셀을 지웁니다.
 *
 * @param size 지우개 크기 (1 = 1픽셀, 2 = 3x3, 3 = 5x5, ...)
 */
class EraserTool(
    ...
    override fun onUp(state: DotKitState, point: Point, color: Int, toolState: ToolState?): CanvasCommand? {
        ...
        // 시작부터 끝까지 지워진 모든 중심 픽셀에 대한 경로 생성
        ...
        val commands = allPixels.mapNotNull { pixel ->
            ...
            if (state.activeLayer?.isInBounds(pixel.x, pixel.y) == true) {
                DrawPixelCommand(layerId, pixel.x, pixel.y, transparentColor, previousColor)
            } else {
                null
            }
        }
    ...
    override fun getPreviewPixels(toolState: ToolState?): List<Pair<Point, Int>> {
        ...
        // 각 픽셀을 투명 색상과 함께 반환
        return allPixels.map { it to transparentColor }
    }

    /**
     * 단일 포인트를 지우개 크기에 따라 확장
     * size=1: 1x1 (단일 픽셀)
     * size=2: 3x3 (radius 1)
     * size=3: 5x5 (radius 2)
     */
